...
__global__ void CompactKernel(const scalar_t *a, scalar_t *out, size_t size,
  ...
  // gid % shape[i] = loop index of i
  // sum(loop index of i * strides[i])  --> all_offset
  if (gid < size) {
    size_t all_offset = offset;
    size_t temp = gid;
    for (int i = shape.size - 1; i >= 0; i--) {
      size_t current_index = temp % shape.data[i];
      temp /= shape.data[i];
      all_offset += current_index * strides.data[i];
    }

    out[gid] = a[all_offset];
  }
  /// END YOUR SOLUTION
}
...
void EwiseSetitem(const CudaArray &a, CudaArray *out,
   ...
   * implement a EwiseSetitemKernel() function, similar to those above, that
...
__global__ void ScalarSetitemKernel(scalar_t val, size_t size, scalar_t *out,
...
void ScalarSetitem(size_t size, scalar_t val, CudaArray *out,
  ...
  ScalarSetitemKernel<<<dim.grid, dim.block>>>(
      val, size, out->ptr, VecToCuda(shape), VecToCuda(strides), offset);
  /// END YOUR SOLUTION
}
